...
public class DemoDataLoader implements CommandLineRunner {
	...
	private void scrutinDemo() {
		...
		String[][] candidatsDemo = {
			{ "moreau@demo.fr", "CAND001", "Alexandre", "Moreau", "Union Nouvelle" },
			{ "fontaine@demo.fr", "CAND002", "Claire", "Fontaine", "Rassemblement Citoyen" },
			{ "belkacem@demo.fr", "CAND003", "Yanis", "Belkacem", "Alliance Progressiste" },
			{ "rousseau@demo.fr", "CAND004", "Émilie", "Rousseau", "Parti Écologiste Populaire" },
		};
		for (String[] c : candidatsDemo) {
			compteDemo(c[0], "root", c[1], false);
		}
		...
		for (Utilisateur utilisateur : utilisateurRepository.findAll()) {
			...
			for (String[] c : candidatsDemo) {
				if (c[0].equals(utilisateur.getEmail())) {
					...
					candidat.setPrenom(c[2]);
					candidat.setNom(c[3]);
					candidat.setParti(c[4]);
